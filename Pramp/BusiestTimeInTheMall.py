...
def find_busiest_period(data):
  ...
  max_visitors = float('-inf')
  max_timestamp = 0
  current_visitors = 0
  
  past_timestamp = data[0][0]
  
  for time_information in data:
    
    current_timestamp = time_information[0]
    visitors_changed = time_information[1]
    
    if past_timestamp != current_timestamp and current_visitors > max_visitors:
      max_visitors = current_visitors
      max_timestamp = past_timestamp     
    
    if time_information[2]:
      current_visitors += visitors_changed
    else:
      current_visitors -= visitors_changed
